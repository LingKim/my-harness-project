...
`spec_reviewer` MUST 在实现完成后读取 change 的 `proposal.md`、`design.md`、`tasks.md`、相关领域 specs、实现 diff 和测试证据，执行 Spec → 代码正向对账与代码 → Spec 反向对账。报告 MUST 保留每条独立要求，不得主观合并或跳过，并为每个判断提供尽可能精确的文件路径和行号。
