...
仓库 SHALL 包含相互独立的 `frontend/` 和 `backend/` 工程；每个工程分别使用所属技术生态的依赖清单、锁定文件或 Wrapper、源码结构及验证命令。

#### Scenario: 开发者检查仓库结构
- **WHEN** 开发者检出仓库
- **THEN** 可以在 `frontend/` 中识别并操作前端工程
- **AND** 可以在 `backend/` 中识别并操作后端工程
...
仓库 Harness 检查 SHALL 在初始化后检测前端、后端、基础设施和 OpenSpec 关键文件是否缺失。

#### Scenario: 基线结构完整
- **WHEN** 在初始化后的仓库中运行 `./scripts/check-harness.sh`
- **THEN** 只有必要的全栈基线和现有 Harness 结构都存在时才报告成功
