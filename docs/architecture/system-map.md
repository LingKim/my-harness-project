...
| CURRENT | 后端账号模块 | `backend/src/main/java/com/heness/project/account` | 注册、登录、会话、账号偏好、旅行上下文与修改密码 |
| CURRENT | 前端账号设置 | `frontend/src/app/[locale]/account/settings` | 旅行上下文、账号与安全、修改密码 |
...
| CURRENT | 账号 API | `backend/src/main/java/com/heness/project/account/api` | 当前账号、旅行上下文、语言偏好与修改密码合同 |
...
| M03 账号与旅行上下文 | `/[locale]/account`、`/[locale]/account/settings` | `/api/v1/accounts`、`/api/v1/sessions`、`/api/v1/accounts/me`、`/api/v1/accounts/me/travel-context`、`/api/v1/accounts/me:change-password` | `account` | 账号、凭据、会话、语言偏好、旅行上下文 | API、领域、持久化、Vitest、Playwright、MySQL | CURRENT |
