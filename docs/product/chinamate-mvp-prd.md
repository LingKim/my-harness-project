...
> 本文档已经完成产品评审，是产品范围和用户行为的当前基线。数据库结构、API 契约和专项安全设计不要求在所有业务开发前一次性冻结，而是在对应功能进入实现前通过 OpenSpec change 逐项确认。
...
1. 当前 AI 免费额度能够满足 MVP 使用，不把额度采购作为启动开发的阻塞项；访客和注册用户的产品限流规则在 AI 功能设计时确认。
2. 用户图片按无限期保存设计；用户主动删除、账号删除和合规删除的清理规则在图片功能设计时确认。
3. 对象存储在开发图片功能时接入，当前不提前建设。
4. P1 运营后台与对应运营治理功能一起开发，不拆成前置工程。

产品名称、首发城市和语言范围沿用本文档已确认基线。双语内容工作流、社区种子回答来源、官方信息核验责任与复核周期等实现细节，在对应功能的 OpenSpec change 中确认，不作为其他业务功能开始开发的全局门禁。

---

## 21. 后续交付门禁

本 PRD 已完成评审。后续按功能模块推进，每个功能在实现前完成自身所需的以下门禁：

1. 创建对应 OpenSpec change，明确该功能的范围、非目标和验收场景。
2. 按该功能实际范围补充页面和交互状态设计。
3. 涉及持久化时确认对应实体、状态、索引和数据生命周期。
4. 涉及 HTTP 接口时确认请求、响应、错误码、认证、授权和幂等规则。
5. 在同一 change 中拆分前端、后端、AI、内容和测试任务，并按依赖顺序小步实现。
6. 高风险 AI 测试集、首批攻略内容、对象存储和运营后台分别在对应功能进入开发时完成专项门禁。

不要求在首个功能开始前冻结整个 MVP 的数据库和 API；但任何具体功能都不得在自身规格、数据、接口、安全和测试边界尚未确认时直接编码。
